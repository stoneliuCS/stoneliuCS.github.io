...
$
The first thing we want to do is break up these words, a process known as tokenization and then create _Vector Embeddings_ from them. Vector Embeddings are ways to preserve the semantic meanings of each token in a higher dimensional _Vector Space_. To get a feel for how vector embeddings work, we can play around with a local embedding model and see what happens when we add or subtract specific words. Adding two vectors and finding the nearest vectors of the resulting sum should combine the semantic meaning of each
```bash
# E("Water") + E("Ship")
